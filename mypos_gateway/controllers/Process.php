...
class Process extends App_Controller
{
    ...
    {
        $oResponse = $this->mypos_gateway->finish_payment($this->input->post());
        ...
        if ($oResponse['IPCMETHOD'] == 'IPCPurchaseCancel') {
            ...
            log_activity('Mypos Payment [InvoiceID: ' . $invoiceid . ' TransactionID: ' . $oResponse['IPC_TRNREF'] . ' Amount: ' . $oResponse['AMOUNT'] . ' Status: UNPAID' . ']');
            redirect(site_url('invoice/' . $invoiceid . '/' . $invoice->hash));
        } else if ($oResponse['IPCMETHOD'] == 'IPCPurchaseNotify') {
            log_activity('Mypos Payment [InvoiceID: ' . $invoiceid . ' TransactionID: ' . $oResponse['IPC_TRNREF'] . ' Amount: ' . $oResponse['AMOUNT'] . ' Status: PAID' . ']');
            $success = $this->mypos_gateway->addPayment(
                [
            ...
            log_activity('Mypos Payment [InvoiceID: ' . $invoiceid . ' Amount: ' . $oResponse['AMOUNT'] . ' Status: CANCELLED' . ']');
        }
    }
}