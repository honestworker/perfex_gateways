...
class Process extends App_Controller
{
    ...
    {
        $oResponse = $this->skrill_gateway->finish_payment($this->input->post());
        ...
        if ($oResponse['IPCMETHOD'] == 'IPCPurchaseCancel') {
            ...
            log_activity('Skrill Payment [InvoiceID: ' . $invoiceid . ' TransactionID: ' . $oResponse['IPC_TRNREF'] . ' Amount: ' . $oResponse['AMOUNT'] . ' Status: UNPAID' . ']');
            redirect(site_url('invoice/' . $invoiceid . '/' . $invoice->hash));
        } else if ($oResponse['IPCMETHOD'] == 'IPCPurchaseNotify') {
            log_activity('Skrill Payment [InvoiceID: ' . $invoiceid . ' TransactionID: ' . $oResponse['IPC_TRNREF'] . ' Amount: ' . $oResponse['AMOUNT'] . ' Status: PAID' . ']');
            ...
            log_activity('Skrill Payment [InvoiceID: ' . $invoiceid . ' Amount: ' . $oResponse['AMOUNT'] . ' Status: CANCELLED' . ']');
        }
    }
}